...
function computeLayout(componentes, X, Y, canvasW, canvasH) {
  ...
  componentes.forEach(comp => {
    if (comp.tipo === "compensado") {
      ...
      const ph   = resolveFormula(comp.formula_comprimento, X, Y) * scale;
      ...
      positions.forEach((px, i) => {
        items.push({ id: `${comp.id}-${i}`, tipo: comp.tipo, comp, px, py: oy, pw: larg, ph });
      });
...
function ComponentPopover({ comp, previewX, previewY, onUpdate, onDelete, onClose }) {
  ...
  // Live preview for sarrafo_vertical
  const emendaPreview = local.tipo === "sarrafo_vertical"
    ? calcularEmendaSarrafo(local, X * 10, Y * 10) // X,Y in mm
    : null;
  ...
  return (
    <div
      className="absolute right-3 top-14 z-30 w-68 bg-white border-2 rounded-2xl shadow-2xl overflow-hidden"
      style={{ borderColor: cor.stroke, width: 272 }}
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
          {grupos.length === 0 ? (
            <div className="text-center py-8 text-[#9CA3AF] text-sm">
              Adicione componentes ao painel primeiro.
            </div>
          ) : grupos.map((grupo, gi) => (
            <div key={gi}>
