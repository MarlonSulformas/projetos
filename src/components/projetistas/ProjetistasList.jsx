...
function StatusDot({ ativo }) {
  return (
    <span className={`w-2 h-2 rounded-full flex-shrink-0 ${ativo ? "bg-green-500" : "bg-[#A1A1AA]"}`} />
  );
}
        ...
        {projetistas.map((p, idx) => {
          ...
          return (
            <motion.div
              ...
              className={`
                relative w-full text-left p-4 rounded-xl border transition-all duration-150 cursor-pointer
                ${isSelected
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      <p className={`text-sm font-medium leading-snug truncate ${isSelected ? "text-[#1D4ED8]" : "text-[#1F1F24]"}`}>
                        {p.nome}
                      </p>
                    </div>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-44 rounded-xl shadow-lg border-[#E5E5E8]">
                    <DropdownMenuItem className="gap-2 text-sm cursor-pointer">
                      <Pencil className="w-3.5 h-3.5 text-[#6B6B72]" />
                      Editar dados
                    </DropdownMenuItem>
                    <DropdownMenuItem className="gap-2 text-sm cursor-pointer">
                      <PowerOff className="w-3.5 h-3.5 text-[#6B6B72]" />
                      {ativo ? "Desativar" : "Ativar"}
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem className="gap-2 text-sm cursor-pointer text-red-500 focus:text-red-500">
